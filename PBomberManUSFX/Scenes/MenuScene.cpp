...
MenuScene::MenuScene(GameManager* _gameManager) : Scene(_gameManager)
{
   ...
   /* startText = std::make_shared<Text>(gameManager->getAssetManager()->getFont(), gameManager->getRenderer(), "INICIO");
    ...
    startClasicText->setPosition(static_cast<int>(gameManager->getWindowWidth() / 2.0f - startCartoonText->getWidth() / 2.0f), background->getHeight() + 70);
    ...
    startCustomText->setPosition(static_cast<int>(gameManager->getWindowWidth() / 2.0f - startClasicText->getWidth() / 2.0f), background->getHeight() + 110);
    ...
    startRealisticText->setPosition(static_cast<int>(gameManager->getWindowWidth() / 2.0f - startCustomText->getWidth() / 2.0f), background->getHeight() + 145);
...
void MenuScene::onMenuItemPress()
{
    switch(currentSelectedMenu)
    {
        ...
        case MenuItem::StartClasic:
            // go to level scene
            gameManager->getSceneManager()->addScene("stage", std::make_shared<StageScene>(gameManager, 2, 0));
            gameManager->getSceneManager()->activateScene("stage");
            break;
        case MenuItem::StartCustom:
            // go to level scene
            gameManager->getSceneManager()->addScene("stage", std::make_shared<StageScene>(gameManager, 3, 0));
            gameManager->getSceneManager()->activateScene("stage");
            break;
        case MenuItem::StartRealistic:
            // go to level scene
            gameManager->getSceneManager()->addScene("stage", std::make_shared<StageScene>(gameManager, 4, 0));
            gameManager->getSceneManager()->activateScene("stage");
            break;
        case MenuItem::Exit:
